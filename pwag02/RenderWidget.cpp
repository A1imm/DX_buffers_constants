...
void RenderWidget::CreateConstantBuffers()
{
	...
	ThrowIfFailed(m_dxDevice->CreateCommittedResource(
		...
		nullptr,
		IID_PPV_ARGS(&ConstantVPBufferGPU)
	));
	...
	ThrowIfFailed(m_dxDevice->CreateCommittedResource(
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
		D3D12_HEAP_FLAG_NONE,
		&CD3DX12_RESOURCE_DESC::Buffer(worldBufferSize),
		D3D12_RESOURCE_STATE_GENERIC_READ,
		nullptr,
		IID_PPV_ARGS(&ConstantWorldBufferGPU)
	));
...
void RenderWidget::UpdateWorldCBuffer()
{
	...
	ConstantWorldBufferGPU->Unmap(0, nullptr);
...
void RenderWidget::UpdateViewProjectionCBuffer()
{
	...
	ConstantVPBufferGPU->Unmap(0, nullptr);
}
...
void RenderWidget::Draw()
{
	...
	m_commandList->SetGraphicsRootConstantBufferView(1, ConstantWorldBufferGPU->GetGPUVirtualAddress());
